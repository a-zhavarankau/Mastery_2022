...
class WriterToFormat:
    ...
    def prepare_data_to_store(self, data_to_prepare: List[Dict]):
        pass
...
class FileCreater:
    def create_file(self, data: List[Dict], handler: Type[WriterToFormat], filename: str) -> None:
...
class WriterCSV(WriterToFormat):
    def prepare_data_to_store(self, data_to_prepare: List[Dict]) -> List[List]:
        data_to_store = []
        titles = [field_name.capitalize().replace("_", " ") for field_name in data_to_prepare[0].keys()]
        data_to_store.append(titles)
        for user in data_to_prepare:
...
class WriterJSON(WriterToFormat):
    def prepare_data_to_store(self, data_to_prepare: List[Dict]) -> List[Dict]:
        data_to_store = data_to_prepare
        return data_to_store
...
class DataFormatter:
    ...
    @staticmethod
    def get_user_tax_val(user: Dict, tax_name: str) -> Union[int, float]:
        try:
